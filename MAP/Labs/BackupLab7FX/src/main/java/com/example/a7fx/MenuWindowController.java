...
public class MenuWindowController implements Initializable {
    ...
    public void buildProgramStateExamples(){
        ...
        InterfaceExpression filename4=new ValueExpression(new StringValue("ex4.in"));
        InterfaceStatement ex7 = new CompoundStatement(new OpenReadFileStatement(filename4),
                new CompoundStatement(new VariableDeclarationStatement("v",new IntType()),
                        new CompoundStatement(new ReadFileStatement(filename4, "v"),
                                new CompoundStatement(new PrintStatement(new VariableExpression("v")),
                                        new CompoundStatement(new InterfaceStatement(new LogicExpression(new VariableExpression("v"), new ValueExpression(new IntValue(2)), ">="),
                                                new CompStmt(new ReadFileStmt(filename4, "v"), new PrintStmt(new VarExp("v"))),
                                                new PrintStmt(new ValueExp(new IntValue(-1)))) {
                                        }, new CloseFileStmt(filename4))))));
        this.programStatesExamples = new ArrayList<>(Arrays.asList(ex1, ex2, ex3, ex4, ex5, ex6));
