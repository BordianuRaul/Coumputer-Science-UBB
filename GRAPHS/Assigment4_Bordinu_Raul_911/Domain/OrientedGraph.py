...
class OrientedGraph:
    ...
    def topological_sort(self):
        ...
        while not pq.empty():
            ...
            for child in self.getOutbound(vertex):
                count[child] -= 1
                if count[child] == 0:
                    pq.put(child)

        if len(sorted_order) < self.get_nr_of_vertices():
            sorted_order = None

        return sorted_order

    def highest_cost_path(self, start, end):
        # Perform a topological sort to get the vertices in the order of the longest paths
        sorted_order = self.topological_sort()
        if sorted_order is None:
            return None, None  # The graph has cycles, cannot find a highest cost path

        # Initialize a dictionary to store the highest cost for each vertex
        highest_cost = {v: float('-inf') for v in sorted_order}
        highest_cost[start] = 0  # The highest cost of the start vertex is 0

        # Initialize a dictionary to store the previous vertex in the highest cost path
        previous_vertex = {}

        # Traverse the vertices in the topological order
        for vertex in sorted_order:
            # Update the highest cost for each outbound neighbor
            for neighbor in self.getOutbound(vertex):
                cost = self.retrieve_info(vertex, neighbor)
                new_cost = highest_cost[vertex] + cost
                if new_cost > highest_cost[neighbor]:
                    highest_cost[neighbor] = new_cost
        ...
        # Return the highest cost path and the highest cost value
        return path, highest_cost[end]
